...
namespace Dapper.Easies
{
    public sealed class DbEntity<T> where T : IDbTable
    {
        ...
        public IOrderedDbQuery<T> OrderBy<TField>(params Expression<Func<T, TField>>[] orderFields) => _provider.From<T>().OrderBy(orderFields);

        public IOrderedDbQuery<T> OrderByDescending<TField>(params Expression<Func<T, TField>>[] orderFields) => _provider.From<T>().OrderByDescending(orderFields);
